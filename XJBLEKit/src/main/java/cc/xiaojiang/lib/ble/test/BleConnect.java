        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            switch (msg.what) {
                ...
                case BleMsg.MSG_DISCONNECTED:
                    ...
                    int status = para.getStatus();
                    if (bleConnectCallback != null) {
                        bleConnectCallback.onDisConnected(xjBleDevice, gatt, status);
                        if (gatt != null) {
                            gatt.close();
                            gatt = null;
                        }
...
//                    mOtaVersionCallback.onVersionSucceed(mOtaInfo);
    ...
    public synchronized BluetoothGatt connect(XJBleDevice xjBleDevice,
                                              boolean autoConnect,
        ...
        if (gatt != null) {
            ...
            XJBleManager.getInstance().getMultipleBluetoothController().removeConnectingBle(BleConnect.this);
            if (bleConnectCallback != null)
                bleConnectCallback.onConnectFail(xjBleDevice, new OtherException("GATT connect " + "exception occurred!"));
        }
        return gatt;
    }
...
//            BleLog.e("write success: " + ByteUtils.bytesToHexString(characteristic.getValue()));
    ...
    public void writeSingle(byte[] payload, int total, int current, byte cmd, byte msgId,
        ...
        BluetoothGattCharacteristic characteristic = gattService.getCharacteristic(UUID.fromString(UUID_XJ_CHARACTERISTIC_WRITE));
