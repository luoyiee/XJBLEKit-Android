...
public class ByteUtils {
    ...
    public static byte[] arrayToBytes(ArrayList<Integer> arrayList) {
        byte byteArray[] = new byte[arrayList.size()];
        for (int i = 0; i < arrayList.size(); i++) {
            byteArray[i] = (byte) arrayList.get(i).intValue();//这样转型成byte
        }
        return byteArray;
    }
    ...
    public static byte[] stringToBytes(String a) {
        String[] arrayList = a.split("[.]");
        ArrayList<String> list = new ArrayList<>(Arrays.asList(arrayList));
        byte[] bytes = new byte[list.size()];
        for (int i = 0; i < list.size(); i++) {
            bytes[i] = (byte) Integer.parseInt(Integer.toHexString(Integer.parseInt(list.get(i))));
        }
        return bytes;
    }
    ...
    public static String bytesToString(byte[] bytes) {
        ...
        for (byte aByte : bytes) {
            builder.append(divider).append(ByteUtils.getUnsignedByte(aByte));
        }
...
//    }
     ...
     * @return
     */
    public static byte[] strTo16(String s) {
    ...
    public static String strTo162(String s) {
        String str = "";
        for (int i = 0; i < s.length(); i++) {
    ...
    public static String string2x(int value) {
    ...
    public static String toHexStringSplit(byte[] bytes) {
        if (bytes == null)
            return "null";
