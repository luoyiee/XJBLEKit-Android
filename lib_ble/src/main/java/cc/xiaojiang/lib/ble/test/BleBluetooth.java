...
public class BleBluetooth {
    ...
    public BleBluetooth(XJBleDevice bleDevice) {
    ...
    public synchronized BluetoothGatt connect(XJBleDevice bleDevice,
                                              boolean autoConnect,
                                              BleConnectCallback callback) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case BleMsg.MSG_DISCONNECTED: {
                    ...
                    XJBleManager.getInstance().getMultipleBluetoothController().removeBleBluetooth(BleBluetooth.this);
                    disconnect();
                    refreshDeviceCache();
                    closeBluetoothGatt();
                    mainHandler.removeCallbacksAndMessages(null);
                ...
                case BleMsg.MSG_CHA_INDICATE_START:
                    mBleAuthCallback.onAuthStep(bleDevice, BleAuthStep.START.getCode());
                    enableIndicate();
                    break;
...
//                    BleLog.i(blePacket.toString());
                    ...
                    switch (blePacket.getCmdType()) {
                        ...
                        case 0x84:
                            ...
                            if (errorCode84 != 0) {
                                mBleSnapshotGetCallback.onResult(errorCode84, null);
                                return;
                            }
                            try {
                                byte[] payloadReal = ByteUtils.subByte(payload, 1, payload.length - 1);
                                mBleSnapshotGetCallback.onResult(0, bytesToRealHexString(payloadReal));
...
//            receivedTotal = value[2] >> 4;
    ...
    public void writeOTA(byte[] payload, byte cmd, byte msgIndex, BleWriteCallback callback) {
        ...
        if (isSplit) {
            new SplitWriter().splitWrite(this, payload, cmd, msgId, true, 50, SPLIT_WRITE_NUM, callback);
        } else {
            mSendTotal = 0;
            mSendCurrent = 0;
...
//        }
    ...
    public void getData(byte[] payload, BleDataGetCallback callback) {
        this.mBleDataGetCallback = callback;
    ...
    public void startBleWifiConfig(byte[] payload, BleWifiConfigCallback
            callback) {
    ...
    public void otaSendData(OtaInfo.ContentBean.ModuleBean bean, int packageIndex) {
        ...
        task(2, bean.getTotalPackageNumbers() - packageIndex, (int index) -> {
            ...
            writeOTA(data, PayLoadUtils.CMD_DOWN_SEND_DATA, (byte) i, null);//循环发包
        });
    }
